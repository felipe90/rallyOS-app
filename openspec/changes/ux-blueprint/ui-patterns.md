...
- **Radius**: `rounded-2xl` o `rounded-3xl` para máxima suavidad.
...
- La confirmación con el servidor (`Supabase`) se maneja en background.

### 2. **Contextual Overlays**
- El registro a un torneo se abre en un `BottomSheet` (vía `gorhom/bottom-sheet`) con el fondo desenfocado.

### 3. **Empty States**
- No hay torneos? Mostrar ilustración de "Cancha Vacía" con un CTA prominente hacia `Discovery`.
